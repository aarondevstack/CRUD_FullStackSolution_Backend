...
func (_q *UserQuery) QueryBlogs() *BlogQuery {
...
func (_q *UserQuery) WithBlogs(opts ...func(*BlogQuery)) *UserQuery {
	query := (&BlogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBlogs; query != nil {
		if err := _q.loadBlogs(ctx, query, nodes,
...
func (_q *UserQuery) loadBlogs(ctx context.Context, query *BlogQuery, nodes []*User, init func(*User), assign func(*User, *Blog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
