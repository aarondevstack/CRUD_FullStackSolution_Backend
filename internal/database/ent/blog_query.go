...
func (_q *BlogQuery) QueryAuthor() *UserQuery {
...
func (_q *BlogQuery) QueryComments() *CommentQuery {
...
func (_q *BlogQuery) WithAuthor(opts ...func(*UserQuery)) *BlogQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *BlogQuery) WithComments(opts ...func(*CommentQuery)) *BlogQuery {
	query := (&CommentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *BlogQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Blog, init func(*Blog), assign func(*Blog, *User)) error {
...
func (_q *BlogQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Blog, init func(*Blog), assign func(*Blog, *Comment)) error {
	...
	query.Where(predicate.Comment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(blog.CommentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "blog_comments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
