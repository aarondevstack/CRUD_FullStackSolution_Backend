...
func (_c *UserCreate) AddBlogIDs(ids ...int) *UserCreate {
	_c.mutation.AddBlogIDs(ids...)
	return _c
}

// AddBlogs adds the "blogs" edges to the Blog entity.
func (_c *UserCreate) AddBlogs(v ...*Blog) *UserCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddBlogIDs(ids...)
}
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.BlogsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
